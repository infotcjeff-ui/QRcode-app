...
import { ScanInDashboardClient } from "@/components/scan/scan-in-dashboard-client";
...
type LoadResult = {
  ...
  // 「點名」頁面需要看到全部校巴與全部學生，以便依保姆的 assigned_bus_id 篩選。
  allBuses: Bus[];
  allStudents: Student[];
};
...
async function load(): Promise<LoadResult> {
  ...
  if (!adminConfigured) {
    return {
      ...
      allBuses: [FALLBACK_BUS],
      allStudents: FALLBACK_STUDENTS,
    };
  }

  const supabase = getSupabaseAdmin()!;

  const [attendantRes, tripRes, busRes, studentsRes, logsRes, busesRes] = await Promise.all([
  ...
  const allBuses = (busesRes.data as Bus[] | null) ?? [FALLBACK_BUS];
